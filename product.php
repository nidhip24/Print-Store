...
<html>
    <head>
        ...
        <title>Product - The Vagad Print Store</title>
...
-->
        ...
        $stmt->close();
        //getting sub category names
        if($stmt = $conn->prepare("SELECT pt.type, pt.img FROM productcontent as pc, product_type as pt WHERE pt.pid=(SELECT id from productcontent WHERE pt.pid=pc.id and product=?) and pt.stat='live' ")){
            $stmt->bind_param("s",$cat);
            $stmt->execute();
            $stmt->bind_result($ptype,$pimg);            
        }else{
            //print_r($conn->error_list);
            echo "no";
        }
        ...
        ?>
            ...
            <!--  Categories    -->
            <!-- <div class="row center" style="margin-top: 30px;margin-bottom: 0px">
                <p class="flow-text">Popular Categories</p>
            </div> -->
            ...
            <?php //} 
                ...
                ?>
                <!-- <div class="col m2 s6">
                    <a href="product.php?cat=<?php //echo $cat; ?>&sub=<?php //echo $ptype; ?>">
                        <div class="card z-depth-1">
                            <div class="card-image">
                                <img class="catimg" src="img/<?php //echo $pimg; ?>" />                    
                            </div>
                            <div class="row nomargin center">
                                <p class="lrmargin black-text truncate"><?php //echo $ptype; ?></p>
                            </div>
                        </div>
                    </a>
                 </div> -->
        ...
        <div class="row container">
                ...
                $sql = "";
                if($sub!=""){
                    $sql = "SELECT p.`id`, `name`, `price`, `img1` FROM `product` as p,`productcontent` as pc WHERE p.pid=pc.id and p.pid=(SELECT id from productcontent WHERE product=?) and p.ptid=(SELECT id from product_type WHERE type=?) and p.stat='live'";                    
                    if($stmt = $conn->prepare($sql)){
                        $stmt->bind_param("ss",$cat,$sub);
                        $stmt->execute();
...
//                        echo "ok";
                    ...
                    $sql = "SELECT p.`id`, `name`, `price`, `img1` FROM `product` as p,`productcontent` as pc WHERE p.pid=pc.id and p.pid=(SELECT id from productcontent WHERE product=?) and p.stat='live' ";                    
                    if($stmt = $conn->prepare($sql)){
                        $stmt->bind_param("s",$cat);
                        $stmt->execute();
                        $stmt->bind_result($id, $name, $price, $img);
...
//                        print_r($conn->error_list);
                ...
                while($stmt->fetch()) {
                    if($i==0){ ?>
                        <div class="row nomargin" style="margin-top: 20px; margin-bottom: 10px"><p class="nomargin" style="font-size: 18px; font-weight: 600">Products</div>
                ...
                <?php } 
                if($i==0){ ?>
                    <div class="col s12 m12 center" style="margin-top:50px;height:50vh;">
