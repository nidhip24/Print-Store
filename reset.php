...
<html>
    <head>
        ...
        <title>Forgot password - The Vagad Print Store</title>
...
-->
        ...
        <div class="row container white" style="margin-top: 20px;min-height: 50vh">
            <div class="col s6 offset-s3 card">
                ...
                <?php if(isset($_GET["token"]) ){ 
                        ...
                        $stmt = $conn->prepare("SELECT `id`, `token` FROM `token` WHERE token=? ");
                        $stmt->bind_param("s", $t);
                        $stmt->execute();
                        $stmt->bind_result($id,$tokn);

                        $flag = -1;
                        
                        while($stmt->fetch()) { ?>
                                <p style="font-weight: 600; font-size: 18px">Enter new password</p>
                                <form id="change-pass-form" action="php/chargepassword.php" method="post" >
                                    <div class="row">                    
                                        ...
                                        <div class="input-field col s10">
                                            <input placeholder="Re type password" id="cnfpassword" name="cnfpassword" type="password" class="validate" required>
                                            <label for="cnfpassword">Confirm password</label>

                                            <input type="text" name="token" value="<?php echo $t?>" hidden="true">
                                        </div>
                                        <div class="col s12">
                                            <button class="waves-effect waves-light btn blue">Change password</button>
                                        </div>
                                    </div>
                                </form>
                            <?php $stmt->close();
                        ...
                        if ($flag==-1) { ?>
                            <h6>Uh sorry! Looks like session is expired!</h6>    
                        <?php }
